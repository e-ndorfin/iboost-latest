...
def index(request):
    ...
    if request.method == 'POST':
        print('test')
        joinclassform = JoinClassForm(request.POST, profile=request.user)
        logger.warning(joinclassform.save())
        logger.warning(joinclassform.save().errors)
        print (joinclassform.save().errors)
